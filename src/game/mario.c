...
static void set_mario_y_vel_based_on_fspeed(struct MarioState *m, f32 initialVelY, f32 multiplier) {
  ...
  //  }
	...
	if (m->squishTimer == 4 || m->quicksandDepth > 1.0f) {
        m->vel[1] *= 0.6f;
    } else if (m->squishTimer == 3 || m->quicksandDepth > 1.0f) {
        m->vel[1] *= 0.7f;
    } else if (m->squishTimer == 2 || m->quicksandDepth > 1.0f) {
        m->vel[1] *= 0.8f;
    } else if (m->squishTimer == 1 || m->quicksandDepth > 1.0f) {
        m->vel[1] *= 0.9f;
    } else if (m->squishTimer == 0 || m->quicksandDepth > 1.0f) {
        m->vel[1] *= 1.0f;
	}
}
...
void squish_mario_model(struct MarioState *m) {
    if (m->squishTimer != 0xFF) {
        ...
        if (m->squishTimer == 0 && m->scaleY < 1 ) {
            vec3f_set(m->marioObj->header.gfx.scale, 1.0f, 1.0f, 1.0f);
        } else if (m->squishTimer == 1 && m->scaleY > 0 && m->scaleY < 2) {
			vec3f_set(m->marioObj->header.gfx.scale, 1.50f, 1.15f, 1.50f);
		} else if (m->squishTimer== 4 && m->scaleY > 1 && m->scaleY < 3) {
			vec3f_set(m->marioObj->header.gfx.scale, 2.25f, 1.20f, 2.25f);
		} else if (m->squishTimer== 3 && m->scaleY > 1 && m->scaleY < 3) {
			vec3f_set(m->marioObj->header.gfx.scale, 2.0f, 1.15f, 2.0f);
		} else if (m->squishTimer== 2 && m->scaleY > 1 && m->scaleY < 3) {
			vec3f_set(m->marioObj->header.gfx.scale, 1.75f, 1.15f, 1.75f);
		}
...
void update_mario_joystick_inputs(struct MarioState *m) {
   ...
   if (m->squishTimer == 0) {
    ...
    } else if (m->squishTimer == 4) {
        m->intendedMag = mag / 3.0f;
    } else if (m->squishTimer == 3) {
        m->intendedMag = mag / 2.75f;
	} else if (m->squishTimer == 2) {
        m->intendedMag = mag / 2.50f;
	} else if (m->squishTimer == 1) {
        m->intendedMag = mag / 2.25f;
	}
