...
void render_hud_mario_lives(void) {
    print_text(GFX_DIMENSIONS_RECT_FROM_LEFT_EDGE(22), HUD_TOP_Y, "DEATHS"); // 'Mario Head' glyph
    print_text_fmt_int(40, 180, "%d", gHudDisplay.lives);
	if (gPlayer1Controller->buttonPressed == L_TRIG && fitnesstoggle == 1) {
		fitnesstoggle=0;
		play_sound_rbutton_changed();
	} else if (gPlayer1Controller->buttonPressed == L_TRIG && fitnesstoggle == 0) {
		fitnesstoggle=1;
		play_sound_rbutton_changed();
	}
	if (fitnesstoggle == 1) {
		print_text(230, 160, "FITNESS");
		print_text_fmt_int(230, 135, "%d", gPoints);
	} else {
	}
}
