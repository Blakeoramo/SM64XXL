...
Gfx *geo_mario_head_scaler(s32 callContext, struct GraphNode *node, UNUSED Mat4 *c) {
    static s16 sMarioAttackAnimCounter = 0;
    struct GraphNodeGenerated *asGenerated = (struct GraphNodeGenerated *) node;
    struct GraphNodeScale *scaleNode = (struct GraphNodeScale *) node->next;
    struct MarioBodyState *bodyState = &gBodyStates[0];

    if (callContext == GEO_CONTEXT_RENDER && cheatDKToggle > 0) {
        //scaleNode->scale = 1.0f;
        scaleNode->scale =
        gMarioAttackScaleAnimation[asGenerated->parameter * 6]
        / 5.0f;
    } else if (callContext == GEO_CONTEXT_RENDER && cheatDKToggle < 1) {
		scaleNode->scale = 1.0f;
		return NULL;
	}
    return NULL;
